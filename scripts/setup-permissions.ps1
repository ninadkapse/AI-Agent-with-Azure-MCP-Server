...
Write-Host "Azure MCP Server - Entra ID Permission Setup" -ForegroundColor Cyan
Write-Host "==============================================" -ForegroundColor Cyan
...
try {
    ...
    if ($azdValues) {
        ...
        Write-Host "Loaded azd environment values." -ForegroundColor Green
    }
} catch {
    Write-Host "Could not load azd values. Will prompt for input." -ForegroundColor Yellow
}
...
function Add-ApiPermission {
    ...
    az ad app permission add --id $AppId --api $ApiId --api-permissions $Permissions 2>$null
...
# =============================================================================
Write-Host "--- Step 1: Adding Mcp.Tools.ReadWrite permission to Client App ---" -ForegroundColor Cyan
...
# =============================================================================
Write-Host ""
Write-Host "--- Step 2: Adding downstream API permissions to Server App ---" -ForegroundColor Cyan
...
az ad app permission admin-consent --id $ENTRA_APP_SERVER_CLIENT_ID 2>$null
if ($LASTEXITCODE -eq 0) {
    Write-Host " ✓" -ForegroundColor Green
} else {
    Write-Host " (may require tenant admin privileges)" -ForegroundColor Yellow
}

Write-Host "  Granting admin consent for client app..." -NoNewline
az ad app permission admin-consent --id $ENTRA_APP_CLIENT_CLIENT_ID 2>$null
if ($LASTEXITCODE -eq 0) {
    Write-Host " ✓" -ForegroundColor Green
} else {
    Write-Host " (may require tenant admin privileges)" -ForegroundColor Yellow
...
# =============================================================================
Write-Host ""
Write-Host "--- Step 4: Setting Identifier URI on Server App ---" -ForegroundColor Cyan
Write-Host "  Setting Identifier URI..." -NoNewline
az ad app update --id $ENTRA_APP_SERVER_CLIENT_ID --identifier-uris "api://$ENTRA_APP_SERVER_CLIENT_ID" 2>$null
if ($LASTEXITCODE -eq 0) {
    Write-Host " ✓ api://$ENTRA_APP_SERVER_CLIENT_ID" -ForegroundColor Green
} else {
...
Write-Host ""
Write-Host "==============================================" -ForegroundColor Cyan
Write-Host "✅ Entra ID Permission Setup Complete!" -ForegroundColor Green
Write-Host "==============================================" -ForegroundColor Cyan
Write-Host ""
Write-Host "Next steps:" -ForegroundColor White
Write-Host "  1. Azure Portal → Client App ($ENTRA_APP_CLIENT_CLIENT_ID) → API permissions"
Write-Host "     Verify 'Mcp.Tools.ReadWrite' permission is listed and granted"
Write-Host "  2. Azure Portal → Server App ($ENTRA_APP_SERVER_CLIENT_ID) → API permissions"
Write-Host "     Verify all downstream API permissions are listed and granted"
Write-Host "  3. Configure the Power Apps custom connector (see README.md)"
Write-Host ""
